...
MANAGER_PROMPTS: Dict[str, str] = {
    "manager-calendar": (
        ...
        "\n\n[REGRA OBRIGATORIA - MEET EH CALENDAR] Pedidos como 'marcar reuniao as 16:00 pelo meet', "
        "'criar meet', 'agendar chamada de video', 'reuniao online' sao SEMPRE eventos de calendario "
        "(calendar.create_event com conferenceData). NUNCA trate 'meet' como uma API separada do Google Meet: "
        "o link do Meet nasce automaticamente do conferenceData do evento. Sempre retorne o link do Meet "
        "(hangoutLink) na resposta quando criar evento com 'meet'."
    ...
    "manager-people": (
        ...
        "[SALVAR/ATUALIZAR CONTATO] Voce NAO tem tool de escrita no Google Contacts (o Composio "
        "people toolkit aqui e somente leitura). Quando o usuario pedir para SALVAR ou ATUALIZAR "
        "um dado de contato (ex: 'guarde o email da Clarissa como principal', 'atualize o telefone'), "
        "chame memory.save_fact(key='contato_{nome}', value='{dado}') para registrar na memoria "
        "pessoal e responda: 'Anotei aqui! 📝 Quando voce pedir o contato dessa pessoa, eu ja trago "
        "esse dado automaticamente.' NAO diga que nao tem acesso a contatos — voce TEM a memoria "
        "como recurso. E use memory.search_facts(key='contato_{nome}') ANTES de buscar o contato, "
        "para reaproveitar dados ja anotados."
    ),
