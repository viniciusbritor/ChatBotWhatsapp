...
phone = '+5511966830020'
phone_h = hashlib.sha256(phone.encode()).hexdigest()[:16]
now = datetime.now(timezone.utc).isoformat()

print(f"phone={phone}")
print(f"phone_hash={phone_h}")

contact_ref = db.collection('contatos').document(phone_h)
contact_ref.set({
    'phone': phone,
    'phone_hash': phone_h,
    'opted_in': True,
...
print(f"Contato criado com ID (phone_hash): {phone_h}")
print(f"Verificacao: {contact_ref.get().to_dict()}")
