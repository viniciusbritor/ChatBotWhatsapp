...
- **Groq Classificador Zero Custo + Remoção ata_worker (13/08/2026)**: `_classify_intent_llm` em `orchestrator.py` atualizado para utilizar **Groq `llama-3.1-8b-instant`** (zero custo, ~100ms) como classificador primário de intenção com fallback para DeepSeek V4 Flash. Remoção completa do job em lote obsoleto `ata_worker`, script de seed `ata-generator` e esteira `cloudbuild-ata-test.yaml`.
