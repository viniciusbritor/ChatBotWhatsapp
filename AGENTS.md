...
- **Composio INTEGRADO (09/08/2026)**: tools `linkedin.*`, `youtube.*` e `googledocs.*` em `tools/{linkedin,youtube,googledocs}_composio.py` via SDK Composio (não confundir com as tools Google nativas `calendar./drive./gmail.` que usam OAuth per-user). **4 camadas obrigatórias para funcionar** (qualquer uma faltando = erro genérico no WhatsApp): (1) `USER_SCOPED_TOOL_PREFIXES` inclui `youtube.`/`linkedin.`/`googledocs.` → `_bind_tool_args` injeta `phone`; (2) wrappers no `tool_registry.py` repassam `phone=kwargs.get("phone","")`; (3) funções passam `user_id` ao `tools.execute()`; (4) `toolkit_versions` pinadas em `tools/_composio_common.py` (11 apps). Schemas reais: YouTube usa `q`/`maxResults` (não query/max_results), GoogleDocs usa `document_id`/`file_id` (não id), LinkedIn exige `author` URN (`urn:li:person:{id}` resolvido via `_resolve_author_urn`). `COMPOSIO_API_KEY` no Secret Manager. Endpoints Connect API: `/api/v1/composio/{status,connect-all,authorize-owner}` (usa `user_ids=[phone]`, plural).
