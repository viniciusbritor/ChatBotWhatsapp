...
- **Auto-exposição de tools + STT real (12/08/2026)**: o jennifier agora usa `tools: null` → `orchestrator._resolve_agent_tools` expõe **todas** as tools do `TOOL_REGISTRY` automaticamente (menos o denylist `INTERNAL_TOOL_IDS` = `image_report.render`, `group.resolve_mention`). **NUNCA manter lista estática de tools no `jennifier.yaml`** — é esse o "trigger automático" de novas tools. **Áudio**: a transcrição (MiniMax STT primário + Gemini fallback) foi ligada no caminho real (`core/audio_pipeline.py::transcribe_envelope_audio`, chamado em `main.py /pubsub/push` antes de `orchestrate`). Antes, o áudio chegava ao DeepSeek como `[audio]` (não transcreve). O `/chat` reusa o mesmo helper.
